...
class GeomGMIFWCartoonForeModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        parser.set_defaults(no_dropout=True)  # default CycleGAN did not use dropout
        parser.set_defaults(dataset_mode='umlvd_ifw_cartoon')
        ...
        if is_train:
            ...
            parser.add_argument('--coherent', type=int, default=0, help='use coherent discriminator')
            parser.add_argument('--lambda_G_A_coh', type=float, default=0.5, help='weight for coherent GAN loss in G')
            parser.add_argument('--coh_use_more', type=int, default=0, help='use more type of samples for coherent discriminator')
            ...
            #parser.add_argument('--select_target12_thre', type=float, default=0.0, help='the probability of using drawing films for targetB1,B2_lm')
            parser.add_argument('--select_noniden_thre', type=float, default=0.9, help='the probability of using non-iden landmark as target')
            # 7. more weight for lip
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            visual_names_A = ['real_A', 'realA_static_warp', 'realA_static_warp2', 'real_A_if_face', 'real_A_if_mask',  'real_A_lm', 'target_B_lm', 'fake_B', 'fake_B_lm_68_vis', 'fake_B_lm_68_vist', 'target_B2_lm', 'fake_B2', 'fake_B2_lm_68_vis', 'fake_B2_lm_68_vist']
            visual_names_B = ['real_B', 'real_B_lm']
        if self.opt.blendbg:
    ...
    def backward_G(self):
        ...
        if self.opt.identity_loss == 1:
            ...
            self.fake_B_gray = 0.299*self.fake_B[:,0,...] + 0.587*self.fake_B[:,1,...] + 0.114*self.fake_B[:,2,...]
            self.fake_B_rep = self.fake_B_gray.repeat(1,3,1,1)
            self.fakeB_static_gray = 0.299*self.fakeB_static[:,0,...] + 0.587*self.fakeB_static[:,1,...] + 0.114*self.fakeB_static[:,2,...]
            self.fakeB_static_rep = self.fakeB_static_gray.repeat(1,3,1,1)

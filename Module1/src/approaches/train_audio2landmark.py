...
class Audio2landmark_model():
    ...
    def __init__(self, opt_parser, jpg_shape=None):
        ...
        if os.path.exists('src/dataset/utils/STD_FACE_LANDMARKS.txt'):
            self.std_face_id = np.loadtxt('src/dataset/utils/STD_FACE_LANDMARKS.txt')
            dump_dir='examples/dump'
        else:
            self.std_face_id = np.loadtxt('Module1/src/dataset/utils/STD_FACE_LANDMARKS.txt')
            dump_dir='Module1/examples/dump'
        ...
        self.eval_data = Audio2landmark_Dataset(dump_dir=dump_dir,
                                                dump_name='random',
                                                status='val',
        ...
        if os.path.exists('src/dataset/utils/STD_FACE_LANDMARKS.txt'):
            ...
            self.anchor_t_shape = np.loadtxt('Module1/src/dataset/utils/STD_FACE_LANDMARKS.txt')
        self.anchor_t_shape = self.anchor_t_shape[self.t_shape_idx, :]

        if os.path.exists(os.path.join('checkpoints', 'emb.pickle')):
            emb_path = os.path.join('checkpoints', 'emb.pickle')
        else:
            emb_path = os.path.join('Module1', 'checkpoints', 'emb.pickle')
    ...
    def __train_face_wo_pos__(self, fls, aus, embs, face_id, smooth_win=31, close_mouth_ratio=.99):
        # only self.C -- Audio2landmark_content
    ...
    #
        ...
        # ''' CALIBRATION '''
        # aus: T*18*80, residual_face_id: T*204
        baseline_pred_fls, _ = self.C(aus[:, 0:18, :], residual_face_id)
        baseline_pred_fls = self.__calib_baseline_pred_fls__(baseline_pred_fls)
        fl_dis_pred = baseline_pred_fls
    ...
    def __train_face_wo_base__(self, fls, aus, embs, face_id, smooth_win=31, close_mouth_ratio=.99):
        # only self.G -- Audio2landmark_pos
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                if(vis_fls):
                    ...
                    import pdb
                    #pdb.set_trace()
                    audio_path = 'examples' if os.path.exists('examples') else os.path.dirname(video_name)
                    Vis(fls=fake_fls_np, filename=video_name.split('\\')[-1].split('/')[-1], fps=62.5,
                        audio_filenam=os.path.join(audio_path, video_name.split('\\')[-1].split('/')[-1]+'.wav'))

    def __train_pass_wo_pos__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False, au_dir=None):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                for j in range(0, inputs_fl.shape[0], seg_bs):
                    ...
                    input_face_id = self.std_face_id
                    # inputs_emb_segments: speaker identity embedding, tile to shape of inputs_emb
                    # inputs_fl_segments: from fl data, landmark 3d
                    # inputs_au_segments: from au data (au[0])

                    fl_dis_pred_pos, input_face_id = \
                        self.__train_face_wo_pos__(inputs_fl_segments, inputs_au_segments, inputs_emb_segments,
                                                           input_face_id)
                ...
                filename = 'pred_fls_{}_{}_nopos.txt'.format(video_name.split('\\')[-1].split('/')[-1], key)
                print('here')
                np.savetxt(os.path.join(self.opt_parser.output_folder, filename), fake_fls_np, fmt='%.6f')
    ...
    def __train_pass_wo_base__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                for j in range(0, inputs_fl.shape[0], seg_bs):
                    ...
                    fl_dis_pred_pos, input_face_id = \
                        self.__train_face_wo_base__(inputs_fl_segments, inputs_au_segments, inputs_emb_segments,
                                                           input_face_id)
                ...
                filename = 'pred_fls_{}_{}_nobase.txt'.format(video_name.split('\\')[-1].split('/')[-1], key)
                print('here')
                np.savetxt(os.path.join(self.opt_parser.output_folder, filename), fake_fls_np, fmt='%.6f')
    ...
    def test_wo_pos(self, au_emb=None, au_dir=None):
        with torch.no_grad():
            self.__train_pass_wo_pos__(au_emb, vis_fls=True, au_dir=au_dir)
    
    def test_wo_base(self, au_emb=None):
        with torch.no_grad():
            self.__train_pass_wo_base__(au_emb, vis_fls=True)

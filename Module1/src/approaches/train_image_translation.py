...
class Image_translation_block():
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False, output_single=False, output_both=False, output_dir='examples', output_landmark=False, landmark_dir='base+pose'):
        import time
        st = time.time()
        self.G.eval()
        ...
        if output_both:
            ...
            writer_g = cv2.VideoWriter('out_g.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
        elif not output_both and not output_single:
            writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256 * 3, 256))
        elif not output_both and output_single:
            writer_g = cv2.VideoWriter('out_g.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
        ...
        for i, frame in enumerate(fls):
            ...
            for j in range(g_out.shape[0]):
                ...
                frame = np.concatenate((ref_in[j], g_out[j], np.expand_dims(img_fl,0)[j].astype(np.float32)/255.0), axis=1) * 255.0
                if writer is not None:
                    writer.write(frame.astype(np.uint8))
                if writer_g is not None:
                    writer_g.write((g_out[j]* 255.0).astype(np.uint8))
                if output_landmark:
                    cv2.imwrite('{}/{}.png'.format(landmark_dir, i), np.expand_dims(img_fl,0)[j])
        ...
        if(filename is None):
            filename = 'v'
        audioname = filename.split('_audio')[0]
        if output_both:
            os.system('ffmpeg -loglevel error -y -i out.mp4 -i {} -pix_fmt yuv420p -strict -2 {}/{}_{}.mp4'.format(
            output_dir+'/'+audioname[9:]+'.wav',
            output_dir, prefix, filename[:-4]))
            os.system('ffmpeg -loglevel error -y -i out_g.mp4 -i {} -pix_fmt yuv420p -strict -2 {}/{}_{}_g.mp4'.format(
            output_dir+'/'+audioname[9:]+'.wav',
            output_dir, prefix, filename[:-4]))
        elif not output_both and not output_single:
            os.system('ffmpeg -loglevel error -y -i out.mp4 -i {} -pix_fmt yuv420p -strict -2 {}/{}_{}.mp4'.format(

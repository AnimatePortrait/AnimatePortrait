...
class Audio2landmark_Dataset(data.Dataset):
    ...
    def __init__(self, dump_dir, dump_name, num_window_frames, num_window_step, status):
        ...
        with open(os.path.join(self.dump_dir, '{}_{}_au.pickle'.format(dump_name, status)), 'rb') as fp:
            print('open', os.path.join(self.dump_dir, '{}_{}_au.pickle'.format(dump_name, status)))
            self.au_data = pickle.load(fp)
        with open(os.path.join(self.dump_dir, '{}_{}_fl.pickle'.format(dump_name, status)), 'rb') as fp:
            print('open', os.path.join(self.dump_dir, '{}_{}_fl.pickle'.format(dump_name, status)))
        ...
        if os.path.exists('src/dataset/utils/MEAN_STD_AUTOVC_RETRAIN_MEL_AU.txt'):
            au_mean_std = np.loadtxt('src/dataset/utils/MEAN_STD_AUTOVC_RETRAIN_MEL_AU.txt')
        else:
            au_mean_std = np.loadtxt('Module1/src/dataset/utils/MEAN_STD_AUTOVC_RETRAIN_MEL_AU.txt')
